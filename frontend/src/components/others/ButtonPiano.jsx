    ...
    return (
        <div className={`btn-group ${styles.piano}`} role="group" aria-label="Basic outlined example">
            {arrayLibelleOccurences.map((currentOccurence, index) => (
                <button key={`btn-${index}`} type="button" className={`btn btn-outline-primary ${currentOccurenceInFocus == index && styles.selected}`} onClick={() => (setterCurrentOccurenceInFocus && setterCurrentOccurenceInFocus(index))}>{currentOccurence}</button>
            ))}
        </div>
    );
}
